            ...
            <div className="flex flex-1 flex-col overflow-hidden">
                ...
                <main className="flex-1 overflow-y-auto pb-32">
                    <div className="w-full px-4 sm:px-6 lg:px-8 py-6 space-y-6">
                        <div className="h-8 w-56 animate-pulse rounded bg-[#1E3A2A]" />
                        <div className="h-4 w-72 animate-pulse rounded bg-[#1E3A2A]" />
                        <div className="space-y-3">
                            {Array.from({ length: 8 }).map((_, i) => (
                                <div key={i} className="flex items-center gap-4 rounded-xl border border-[#1E3A2A] bg-[#162B20] p-4">
                                    <div className="h-5 w-5 animate-pulse rounded bg-[#1E3A2A]" />
                                    <div className="h-4 flex-1 animate-pulse rounded bg-[#1E3A2A]" />
                                    <div className="h-6 w-16 animate-pulse rounded-full bg-[#1E3A2A]" />
                                </div>
                            ))}
                        </div>
                    </div>
